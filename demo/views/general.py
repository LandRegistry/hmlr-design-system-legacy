    ...
    if current_app.config.get("DEPENDENCIES") is not None:
        for dependency, value in current_app.config.get("DEPENDENCIES").items():
            ...
            if depth > 0:
                ...
                service = {"name": dependency, "type": "http"}
                try:
                    ...
                    current_app.logger.error(
                        "Connection Aborted during health cascade on attempt to connect to {};"
                        " full error: {}".format(dependency, e)
                    )
                    ...
                    current_app.logger.error(
                        "Unknown error occured during health cascade on request to {};"
                        " full error: {}".format(dependency, e)
                    )
                    service["status"] = "UNKNOWN"
                    overall_status = 500
